...
def forget_pass():
    def change_password():
        if user_entry.get()=='' or newpass_entry.get()=='' or confirmpass_entry.get()=='':
            ...
            con = pymysql.connect(host='localhost', user='root', password='Umesh706', database='userdata') 
            mycursor = con.cursor()
    ...
    heading_label = Label(window, text='RSEST PASSWORD', font=('arial', '18' ,'bold'),
                          bg='white', fg='magenta2')
    heading_label.place(x=480, y=60)

    userLabel= Label(window, text='Username' , font= ('arial', 11, 'bold'), bg='white', fg='orchid1')
    ...
    Frame (window, width=250, height=2, bg='orchid1').place(x=478, y=340)

    submitButton = Button(window, text='Submit', bd=8, bg= 'magenta2', fg='white', font=('Open Sans', '16', 'bold'), 
...
def login_user():
    if usernameEntry.get()=='' or passwordEntry.get()=='':
        ...
        try:
            con=pymysql.connect(host='localhost',user='root',password='Umesh706')
            mycursor=con.cursor()
        except:
...
loginButton=Button(login_window,text='Login',font=('Open Sans',16,'bold'),fg='white',bg='firebrick1',
                   activebackground='firebrick1',activeforeground='white',cursor='hand2',bd=0,width=19,command=login_user)
...
singuplabel=Label(login_window,text='Dont have an account',font=('Open Sans',9,'bold'),fg='firebrick1',
              bg='white')
singuplabel.place(x=590,y=500)

newaccountButton=Button(login_window,text='Create New One',font=('Open Sans',9,'bold underline'),fg='blue',
                        bg='white',activebackground='white',activeforeground='white',cursor='hand2',bd=0,command=signup_page)
