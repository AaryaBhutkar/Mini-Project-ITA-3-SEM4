...
connector.execute(
	'CREATE TABLE IF NOT EXISTS ExpenseTracker (ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, Date DATETIME, Payee TEXT, Description TEXT, Amount FLOAT, ModeOfPayment TEXT, Expenditure TEXT)'
)
connector.commit()
...
def view_expense_details():
	global table
	global date, payee, desc, amnt, MoP, Exp 
	...
	date.set_date(expenditure_date) ; payee.set(values[2]) ; desc.set(values[3]) ; amnt.set(values[4]) ; MoP.set(values[5], Exp.set(values[6]))


def clear_fields():
	global desc, payee, amnt, MoP, date, table, Exp

	today_date = datetime.datetime.now().date()

	desc.set('') ; payee.set('') ; amnt.set(0.0) ; MoP.set('Cash'), date.set_date(today_date), Exp.set('Food')
...
def add_another_expense():
	...
	if not date.get() or not payee.get() or not desc.get() or not amnt.get() or not MoP.get() or not Exp.get():
		mb.showerror('Fields empty!', "Please fill all the missing fields before pressing the add button!")
	else:
		connector.execute(
		'INSERT INTO ExpenseTracker (Date, Payee, Description, Amount, ModeOfPayment, Expenditure) VALUES (?, ?, ?, ?, ?, ?)',
		(date.get_date(), payee.get(), desc.get(), amnt.get(), MoP.get(), Exp.get())
		)
		connector.commit()
...
def edit_expense():
	...
	def edit_existing_expense():
		global date, amnt, desc, payee, MoP, Exp
		global connector, table

		current_selected_expense = table.item(table.focus())
		contents = current_selected_expense['values']

		connector.execute('UPDATE ExpenseTracker SET Date = ?, Payee = ?, Description = ?, Amount = ?, ModeOfPayment = ? WHERE ID = ?, Expenditure = ? WHERE ID = ?',
		                  (date.get_date(), payee.get(), desc.get(), amnt.get(), MoP.get(), Exp.get(), contents[0]))
		connector.commit()
...
def selected_expense_to_words():
	...
	message = f'Your expense can be read like: \n"You paid {values[4]} to {values[2]} for {values[3]} on {values[1]} via {values[5]} for {values[6]}"'

	mb.showinfo('Here\'s how to read your expense', message)


def expense_to_words_before_adding():
	global date, desc, amnt, payee, MoP, Exp

	if not date or not desc or not amnt or not payee or not MoP or not Exp:
		mb.showerror('Incomplete data', 'The data is incomplete, meaning fill all the fields first!')

	message = f'Your expense can be read like: \n"You paid {amnt.get()} to {payee.get()} for {desc.get()} on {date.get_date()} via {MoP.get()} for {Exp.get()}"'
...
dataentery_frame_bg = 'deep sky blue'
buttons_frame_bg = 'royal blue'
hlb_btn_bg = 'turquoise'
...
Label(data_entry_frame, text='Expenditure:', font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=340)
dd2 = OptionMenu(data_entry_frame, MoP, *['Food', 'Accessory', 'Appliance', 'Miscelleneous'])
dd2.place(x=160, y=350)     ;     dd2.configure(width=10, font=entry_font)
...
# Treeview Frame
table = ttk.Treeview(tree_frame, selectmode=BROWSE, columns=('ID', 'Date', 'Payee', 'Description', 'Amount', 'Mode of Payment', 'Expenditure'))
...
table.heading('Expenditure', text='Expenditure', anchor=CENTER)
...
table.column('#7', width=125, stretch=NO)  # Expenditure
